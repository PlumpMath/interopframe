...
public class CodeGeneratorFractal extends CodeGeneratorMethods implements ICodeGenerator {
	private VelocityEngine ve;
	private IBindingGenerator bind;
	private Parameters p;
	
	public CodeGeneratorFractal(Parameters p, IBindingGenerator bind) {
		this.p = p;
		this.bind = bind;
	...
	public StringWriter getWriterSkeleton() {
		Template t = this.ve.getTemplate("lib/templates/SkeletonTemplate_Fractal.vm");		
	...
	public StringWriter getWriterProxy() {
		Template t = this.ve.getTemplate("lib/templates/ProxyTemplate_Fractal.vm");
	...
	public ArrayList<String> mountSkeletonImplMethods(String interfaceName, String className) {
		...
		for (int i = 0; i < methods.size(); i++) {
			...
			if (returnType != "void") {
				body += "		return this.i" + className + "." + nameMethod + "(";
			} else {
				body += "		this.i" + className + "." + nameMethod	+ "(";
			}
